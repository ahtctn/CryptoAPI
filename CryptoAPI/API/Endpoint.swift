...
enum Endpoint {
    case fetchCoins(url: String = "/v1/cryptocurrency/listing/latest")
    ...
    private var url: URL? {
        ...
        components.path = self.path ///v1/cryptocurrency/listing/latest
...
extension URLRequest {
    mutating func addValues(for endpoint: Endpoint) {
        switch endpoint {
        case .fetchCoins:
            self.setValue(HTTP.Headers.Value.applicationJSON.rawValue, forHTTPHeaderField: HTTP.Headers.API_KEY.contentType.rawValue)
            self.setValue(Constants.API_KEY, forHTTPHeaderField: HTTP.Headers.API_KEY.apiKey.rawValue)
        }
    }
}
